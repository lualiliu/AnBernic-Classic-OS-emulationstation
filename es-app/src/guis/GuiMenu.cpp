...
GuiMenu::GuiMenu(Window *window) : GuiComponent(window), mMenu(window, "主菜单(MAIN MENU)"), mVersion(window)
{
	...
	*/
	...
	if (isFullUI)
		addEntry("界面设置(UI SETTINGS)", 0x777777FF, true, [this] { openUISettings(); });
	/*
	if (isFullUI)
...
{
	auto s = new GuiSettings(mWindow, "界面设置(UI SETTINGS)");
	...
	if (!themeSets.empty())
	{
		...
		auto theme_set = std::make_shared<OptionListComponent<std::string>>(mWindow, "主题设置(Theme Setting)", false);
		for (auto it = themeSets.cbegin(); it != themeSets.cend(); it++)
			theme_set->add(it->first, it->first, it == selectedSet);
		s->addWithLabel("主题设置(Theme Setting)", theme_set);
...
void GuiMenu::openOtherSettingsEN()
{
