	...
	mSystem(system), mMenu(window, "OPTIONS"), fromPlaceholder(false), mFiltersChanged(false)
{
	...
	if (!fromPlaceholder) {
		...
		char curChar = (char)getGamelist()->getCursor()->getSortName()[0];
		...
		for (char c = startChar; c <= endChar; c++)
		{
			...
			for (auto file : files)
			{
				...
				char candidate = (char)file->getSortName()[0];
				if (c == candidate)
				{
...
void GuiGamelistOptions::jumpToLetter()
{
	...
	while(max >= min)
	{
		...
		char checkLetter = (char)files.at(mid)->getSortName()[0];
		
		if(checkLetter < letter)
			min = mid + 1;
		//else if(checkLetter > letter || (mid > 0 && (letter == toupper(files.at(mid - 1)->getSortName()[0]))))
		else if(checkLetter > letter || (mid > 0 && (letter == files.at(mid - 1)->getSortName()[0])))
			max = mid - 1;
		else
